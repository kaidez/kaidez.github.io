...
<ol>
  <li>
    The <code>main</code> property which is optional here. It's the entry point for the webpack build, which woold work if this property wasn't listed here.  But doing so is a <em>de facto</em> best practice for webpack.
  </li>
  <li>
    The <code>scripts</code> property which contains two tasks: <code>build</code> which builds out the production-ready JavaScript code via webpack and <code>watch</code>, which watches for changes to the your <code>.js</code> source files and runs that said build.  These source fiels are in the <code>js-build</code> directory listed above and can be run using either yarn or npm...I used yarn.
  </li>
</ul>
